...
RAZORPAY_CAPTURED_WEBHOOK_DELAY_SECONDS = 7
RAZORPAY_WEBHOOK_NOTIFY_EMAIL = "dominic.v@pleasantbiz.com"
...
def _find_razorpay_integration_request(token=None, payment_id=None, order_id=None):
	"""Resolve Integration Request for Razorpay webhook processing."""
	if token:
		try:
			doc = frappe.get_doc("Integration Request", token)
			if doc.integration_request_service == "Razorpay":
				return doc
		except frappe.DoesNotExistError:
			pass
	...
	integration_requests = frappe.get_all(
		...
		fields=["name", "data", "reference_doctype", "reference_docname", "status"],
	)

	for ir in integration_requests:
		ir_data = json.loads(ir.data) if ir.data else {}
		if (
			(payment_id and ir_data.get("razorpay_payment_id") == payment_id)
			or (order_id and (ir_data.get("razorpay_order_id") == order_id or ir_data.get("order_id") == order_id))
		):
			return frappe.get_doc("Integration Request", ir.name)

	return None


def _add_invoice_webhook_comment(reference_doctype, reference_docname, comment_text):
	"""Add webhook processing comments to the reference document when possible."""
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), "Razorpay webhook comment failed")


def _send_razorpay_webhook_email(subject, message):
	"""Send operational visibility email for webhook processing."""
	try:
		frappe.sendmail(
			recipients=[RAZORPAY_WEBHOOK_NOTIFY_EMAIL],
			subject=subject,
			message=message,
			now=True,
		)
	except Exception:
		frappe.log_error(frappe.get_traceback(), "Razorpay Webhook Email Error")
...
@frappe.whitelist(allow_guest=True)
def payment_captured_webhook():
	"""
	Backup webhook endpoint for Razorpay payment.captured events.

	Primary success path remains callback -> verify_payment. This endpoint waits
	a short interval before processing so callback has priority, then applies
	idempotent checks before invoking the existing authorization path.
	"""
	try:
		...
		if not payment_id:
			return {"success": False, "error": "payment_id missing from payload"}

		token = notes.get("token") if isinstance(notes, dict) else None
		integration_request = _find_razorpay_integration_request(
			token=token,
		...
		if not integration_request:
			frappe.log_error(
				f"Razorpay payment.captured: Integration Request not found for payment_id={payment_id}, order_id={order_id}",
		...
		if integration_request.status in ("Completed", "Authorized"):
			...
			return {"success": True, "message": "Already processed", "status": integration_request.status}

		settings = frappe.get_doc("Razorpay Settings")
		company = ir_data.get("company") or (notes.get("company") if isinstance(notes, dict) else None)
		merchant_name = ir_data.get("custom_merchant_name") or (
			notes.get("merchant_name") if isinstance(notes, dict) else None
		)
		creds = settings.get_credentials(data=ir_data, company=company, merchant_name=merchant_name)

		if signature and creds.api_secret:
			try:
				settings.verify_signature(payload_bytes.decode("utf-8"), signature, creds.api_secret)
			except frappe.PermissionError:
				frappe.log_error(
					"Razorpay payment.captured signature verification failed",
					"Razorpay Captured Webhook",
				)
		...
		if integration_request.status in ("Completed", "Authorized"):
			_add_invoice_webhook_comment(
				...
				(
					"<b>Razorpay Captured Webhook Skipped</b><br>"
					"Reason: Callback path completed payment before webhook backup processing.<br>"
					f"Integration Request: {integration_request.name}<br>"
					f"Payment ID: {payment_id}"
				),
			)
			return {"success": True, "message": "Already completed by callback", "status": integration_request.status}

		pre_outstanding = None
		if reference_doctype == "Sales Invoice" and reference_docname:
			try:
				invoice_doc = frappe.get_doc(reference_doctype, reference_docname)
				invoice_doc.reload()
				pre_outstanding = invoice_doc.outstanding_amount
				if pre_outstanding <= 0:
					_add_invoice_webhook_comment(
						...
						(
							"<b>Razorpay Captured Webhook Skipped</b><br>"
							"Reason: Invoice already paid before backup webhook processing.<br>"
							f"Integration Request: {integration_request.name}<br>"
							f"Payment ID: {payment_id}"
						),
					)
				...
				frappe.log_error(frappe.get_traceback(), "Razorpay webhook pre-check failed")

		ir_data = json.loads(integration_request.data) if integration_request.data else {}
		user = (notes.get("user") if isinstance(notes, dict) else None) or ir_data.get("user") or ""
		if user and user != "Guest" and frappe.session.user == "Guest":
			try:
				frappe.set_user(user)
				frappe.local.login_manager.login_as(user)
			except Exception:
				frappe.log_error(frappe.get_traceback(), "Razorpay webhook session restore error")

		ir_data.update(
			{
				"razorpay_payment_id": payment_id,
				"razorpay_order_id": order_id or ir_data.get("razorpay_order_id") or ir_data.get("order_id"),
				"razorpay_webhook_event": event,
				"razorpay_webhook_status": payment_entity.get("status"),
				"razorpay_webhook_amount": payment_entity.get("amount"),
				"user": user or frappe.session.user,
				"webhook_source": "payment_captured_webhook",
			}
		)
		...
		controller.data.razorpay_payment_id = payment_id
		controller.data.razorpay_order_id = ir_data.get("razorpay_order_id")
		controller.data.reference_doctype = reference_doctype
		controller.data.reference_docname = reference_docname
		result = controller.authorize_payment()

		post_outstanding = None
		if reference_doctype == "Sales Invoice" and reference_docname:
			try:
				invoice_doc = frappe.get_doc(reference_doctype, reference_docname)
				invoice_doc.reload()
				post_outstanding = invoice_doc.outstanding_amount
			except Exception:
				frappe.log_error(frappe.get_traceback(), "Razorpay webhook post-check failed")

		_add_invoice_webhook_comment(
			reference_doctype,
			reference_docname,
			(
				"<b>Razorpay Captured Webhook Processed</b><br>"
				f"Integration Request: {integration_request.name}<br>"
		...
		if result.get("status") in ("Authorized", "Completed", "Verified"):
			_send_razorpay_webhook_email(
				subject=f"Razorpay Webhook: payment.captured - {result.get('status')}",
				message=(
					"<h3>Razorpay Captured Webhook Processed</h3>"
					f"<p><strong>Integration Request:</strong> {integration_request.name}</p>"
					f"<p><strong>Payment ID:</strong> {payment_id}</p>"
					f"<p><strong>Order ID:</strong> {order_id or 'N/A'}</p>"
					f"<p><strong>Result Status:</strong> {result.get('status')}</p>"
					f"<p><strong>Reference:</strong> {reference_doctype or 'N/A'} - {reference_docname or 'N/A'}</p>"
					f"<p><strong>Outstanding (before):</strong> {pre_outstanding if pre_outstanding is not None else 'N/A'}</p>"
					f"<p><strong>Outstanding (after):</strong> {post_outstanding if post_outstanding is not None else 'N/A'}</p>"
					"<p><em>This was backup webhook processing after callback delay.</em></p>"
				),
			)
		...
		frappe.log_error(
			...
			"Razorpay Captured Webhook Error",
		)
		return {"success": False, "error": str(e)}
